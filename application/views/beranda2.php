...
<style>
  ...
  @media only screen and (max-width: 1281px) {
    ...
    .btn-menu {
      ...
      background: conic-gradient(#d7d7d7,
          #c3c3c3,
          #cccccc,
          #c6c6c6,
          #d7d7d7,
...
<body style="background-color:#1c222c">
  ...
  <div>
    <span id="enableWifi" style="color: white; margin: -1px 22px; float:right; font-size:13.5px; font-weight:700;"><i class="fas fa-wifi">&nbsp;</i><span id="wifi">Not Configured</span></span>
    <span id="enableLAN" style="color: white; margin: -1px 0px; float:right; font-size:13.5px; font-weight:700;"><i class="fas fa-network-wired">&nbsp;</i><span id="lan">Not configured</span></span>
  </div>
  ...
  <div class="fixed-bottom d-flex justify-content-between align-content-md-stretch bg-dark" style="padding-top:1px;padding-bottom:1px;">
    ...
    <a id="btn-home" class="flex-fill btn  text-center text-success" onclick="gotohome()">
    ...
    <a class="flex-fill btn text-center text-secondary" onclick="gotoconfig()" id="nav-config">
  ...
  <script>
    ...
    $.ajax({
      ...
      if (data[0].enableIPLAN == 1) {
        document.getElementById("enableLAN").style.display = "block";

      } else {
        document.getElementById("enableLAN").style.display = "none";
      }
    });
    ...
    $.ajax({
      ...
      let data = (msg)
      // alert(data[0].enableIPWifi);
      if (data[0].enableIPWifi == 1) {
        document.getElementById("enableWifi").style.display = "block";
        document.getElementById("enableLAN").style.margin = "-1px 0px";
         

      } else {
        document.getElementById("enableWifi").style.display = "none";
        document.getElementById("enableLAN").style.margin = "-1px 22px";
      }
    });
    ...
    $.ajax({
      method: "get",
      url: '../backend/webapi/get_network.php?type=wlan'
    }).done(function(msg) {
      $("#wifi").text(msg.ip)
      // let data = JSON.parse(msg)
      if (msg.IP == null) {
        document.getElementById("wifi").innerHTML = "Not Configured";
      } else {
        document.getElementById("wifi").innerHTML = msg.IP;
      }
      // alert(msg.IP);
    });
    ...
    $.ajax({
      method: "get",
      url: '../backend/webapi/get_network.php?type=eth'
    }).done(function(msg) {
      $("#lan").text(msg.ip)
      // let data = JSON.parse(msg)
      if (msg.IP == null) {
        document.getElementById("lan").innerHTML = "Not Configured";
      } else {
        document.getElementById("lan").innerHTML = msg.IP;
      }
      // alert(msg.IP);
    });
