...
def writeRegister():
    ...
    GPIO.output(RCLK_Pin,1)
	 #sleep(0.001)
	 #IO.output(RCLK_Pin,0)
	 #print(i)
def setRegister(index,value):
	global registers
                    ...
                    for j in GP:
                        if(jmlPort<DASPORT):
                            for k in range(8):
                                ...
                                #print(switch)
                                setRegister(port-1,(switch))
                                if dataChannel[count]["invert"]==1:
                                    switch=self.inverts(switch)
                                if waktu[count]==0:
                                    if(switch==1 or switch==0):
                                        prdata2+="PORT"+str(changP(port))+":"+ str(switch)	
                                        prdata+=str(self.changeVal(switch))+"*"
                                    if(switch !=None):
                                        if(switch !=kondisiOld[count]):
                                            print ("Port:"+str(changP(port))+":"+str(switch))
                                            kondisiOld[count]=switch
                                            if(switch==1):
                                                flagKondisi[count]=1                                  
                                            dataNotif.append({"port":changP(port),"cond":kondisiOld[count],"date": getDate(),"flagCond":flagKondisi[count]})
                                else:                                
