...
class SecondaryLoop:
    ...
    def readPower(self,pwr):
        #fungsi read PowerMeter
        global countRead
        global countModbusId
        global idModbusArr
        global typeModbusArr
        #print(countRead[0])
        #idM= pwr.idModbus[pwr.countModbus[1]]
       # print(idM)
        #Type=int(pwr.type[pwr.countModbus[1]])
        
        idM=idModbusArr[countModbusId]
        
        Type=typeModbusArr[countModbusId]
       # print("Type:",Type)
        #print("Type",Type)
        ...
        if(countRead>1):
            ...
            if(Type ==0):
               # pwr.req[0][0]=int(idM)
                ...
                #pwr.req2[0].pop()
            dt=ser.read(300)
            print("output Serial",dt,"type:",Type)
            if(len(dt)>5):
            ...
            if(pwr.countModbus[0]==len(pwr.req)):
                ...
                try:
                    ...
                    for i in range(len(keys_jsonRes)):
                        ...
                        mqtt.send(client,str(res),topic)   #aktif
                        mqtt.send(client2,str(res),topic)   #aktif
                    ...
                    countModbusId+=1
                    if(countModbusId>=len(idModbusArr)):
                        countModbusId=0
                    #reconnectMqtt=0
...
class config():
    ...
    #################config Serial ###########
        ...
        ser=serial.Serial('/dev/ttyAMA0', baudrate = baud,parity=par,  bytesize=8, timeout=0.5)
     ...
     ################Config Modbus ##################
       ...
       # typeModbusArr=[]
        for x,y in enumerate(dt):
            idModbusArr.append(int(y["idmodbus"]))
            typeModbusArr.append(int(y["modbus_type"]))
        print("Type",typeModbusArr)
        pwr.Helper(0).resetJsonres()   
