...
#pool = Pool(host='localhost', port=3306, user='root', password='', db='alat')
...
class requestData:
    def __init__(self,index):
        self.index = index
    def sendNotif(kode_mesin,lokasi,nama_alat,nama_mesin,port,tanggal,status,kondisi,ipServer,api,chatid):
        try:
...
class readDb:
    ...
    def readLog(x,jml):
        ...
        try:
            ...
            print("eror query")
    def readLogPage(x,jml):
       
        hal = int(jml)*15
        query=f"select tanggal as dta ,notif_list.* from notif_list ORDER BY tanggal desc limit {str(hal)} ,15"
        #print(query)
        try:
            results =queryData(x,query)
            return results
...
class updDb:
    #updateChannelPort
    def updateChannelPort(x,name,port,delay,high,low,desc,check_high,check_low,check_invert):
        query=f"update mesin,m_mesin \
                set mesin.nama_alat='{name}',\
                mesin.delay = {delay},\
                mesin.high ='{high}',\
                mesin.low ='{low}',\
                mesin.detail ='{desc}',\
                mesin.tel_high = {check_high},\
                mesin.tel_low = {check_low},\
                mesin.invert = '{check_invert}',\
                m_mesin.cfg=1\
            where mesin.port = {port} "
        try:
            results =queryData(x,query)
            return results
        except:
    ...
    def updateDelay(x,delay):
        query=f"update m_mesin set  \
                    system_delay  = '{delay}'"             
        try:
            results =queryData(x,query)
            return results
    ...
    def updateDisplay(x,displayed_port,zoom_w,zoom_h,f_size,display_time):
        query=f"update m_mesin set  \
        ...
        try:
            ...
            print("query Error"+NameError)
    def updateWifi(x,ssid,password):
        query=f"update wifi set  \
                    ssid= '{ssid}',\
                    pass = '{password}',\
                    flag    = 1 "
        try:
            ...
            print("query Error"+NameError)
    def updateNetwork(x,server,ip,net,gtw,dns,dhcp):
        query=f"update network set  \
                    ipserver= '{server}',\
                    iplocal = '{ip}',\
                    netmask = '{net}',\
                    dns     = '{dns}',\
    ...
    def updMesin(x,kondisi,flag_kondisi,port):
        ...
        try:
            ...
            print("query Error"+NameError)
    def rstMesin(x):
        query="update mesin,m_mesin set mesin.flag_kondisi = 0 ,mesin.kondisi=0, m_mesin.reset=1,m_mesin.mute=1,m_mesin.ack=0"
        try:
           results =queryData(x,query)
           return results
            #print("db update")
    ...
    def updM_mesin(z,x,y):
        ...
        try:
            ...
            print("query Erro")
    def updDeviceConfig(z,nama,loc):
        query=f"UPDATE m_mesin set nama = '{nama}' , lokasi = '{loc}',nama_gi ='{loc}'" 
        try:
            results =queryData(z,query)
            return results
        except:
...
class delDb:
    def delNotifList(x):
        query=f"delete from notif_list"
        #print(query)
        try:
            results =queryData(x,query)
            return results
